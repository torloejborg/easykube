...
Now temporal claims it’s installed, but Postgres and the ingress controller were also installed. This happened because of the xref:addons.adoc#addons-dependencies["dependsOn"] declaration in the addon.

If all went well, you should be able to visit the Temporal UI at https://temporal.localtest.me

Try adding temporal again with $ easykube add temporal. You get this result:
...
TIP: By passing --force or -f you force the entire dependency tree to be added again. During development, one addon is typically iterated over, so it’s natural to use $ easykube add foo --force --nodepends or $ easykube add -fn.

That’s basically it! You can now build or use software catalogues that run on a local Kubernetes single-node cluster.

== Immutability

Because easykube runs in Docker (or a compatible runtime), it is not possible to open new NodePorts in the cluster and expect them to be available—Docker is immutable by design.

So, if you introduce a new service or switch to a different addon repository, the cluster must be recreated. Your persistent data survives across cluster creation/deletion.
