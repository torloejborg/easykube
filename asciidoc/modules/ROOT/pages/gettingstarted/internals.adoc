...
The local registry is a https://zotregistry.dev[zot] instance, the local image cache. You can use the registry as a pull through registry (if enabled), or push images directly into it (default behavior)

CAUTION: By default, zot is not configured for sync (it will not check upstream images), depending on your use-case this can be enabled. However, we recommend enabling this feature if you primarily use private registries - otherwise Zot can be aggressive in its communication with upstreams, and incur rate-limits, this is especially true for dockerhub without a personal access token.
...
Whenever your easykube cluster is recreated, there is no need to pull images from the internet. The images are cached, and will be pulled from the local registry.
...
TIP: To enable Zot's image sync feature, edit the `~/.config/easykube/zot-config.json` file, look for the "sync->enable" section, and change to true - then restart the zot-registry docker container.
...
WARNING: Currently, ext services can only reside in the default namespace.
