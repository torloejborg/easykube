...
Please note that pre-built easykube binaries do exist for Windows, Linux, and macOS (Binaries for macOS are not signed with Apple). Easykube can run on Windows, but it's recommended to run easykube in WSL2 or a pure Linux environment. Apple users should build from source.

Starting clean, we recommended to use the Nix approach. (You can download a binary, but external binary dependencies must be explicitly managed by **yourself**)
...
To contribute documentation, invoke the third shell.
...
=== Certificates
After installation, Easykube is configured to use a default SSL certificate. The `ingress` addon and the `zot-registry` container are wired to use this certificate.
